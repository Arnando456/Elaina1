...
case 'attp':
                 if (!isRegistered) return reply( ind.noregis())
					...
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchJson(`https://api.vhtear.com/textxgif?text=${teks}&apikey=AR-TEAM12`, {method: 'get'})
...
case 'tts':
       ...
       case 'hartatata2': // Update MrG{108P}*		
		...
		case 'hartatata3': // Update MrG{108P}*
				...
				bh = body.slice(12)
				reply(ind.wait())
				bh = await getBuffer(`https://api.vhtear.com/hartatahta?text=${bh}&apikey=AR-TEAM12`)
				client.sendMessage(from, bh, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
				case 'imagetext':
                ...
                imageToBase64(`https://api.vhtear.com/textxgif?text=${args[0]}&apikey=AR-TEAM12`).then((response) => {var buf = Buffer.from(response, 'base64');
