...
case 'bneon':
         ...
         case 'blackpink':
                ...
                case 'christmas':
                    ...
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/textprome/${command}?apikey=RiuApikey&text=${txt}`)
                   client.sendMessage(from, buffer, image, { quoted: mek })
                    break
case 'lk21':
                    if (args.length == 0) return reply(`Usage: ${prefix + command} query\nExample: ${prefix + command} Transformer`)
                    query = args.join(" ")
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/lk21?apikey=RiuApikey&query=${query}`)
            ...
            case 'bitly':
               ...
               client.updatePresence(from, Presence.composing) 
				...
				case 'ytmp3':
					...
					if (args.length < 1) return reply('Urlnya mana um?')
					ini_link = args[0]
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/ytaudio?apikey=RiuApikey&url=${ini_link}`)
                    get_result = get_result.result
                    txt = `Title : ${get_result.title}\n`
                    txt += `Uploader : ${get_result.uploader}\n`
                    txt += `Duration : ${get_result.duration}\n`
                    ...
                    if (isLimit(sender)) return reply(ind.limitend(pusname))
					query = args.join(" ")
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/ytplay?apikey=RiuApikey&query=${query}`)
                    get_result = get_result.result
                    ...
                    baby.sendMessage(from, get_audio, video, { mimetype: 'video/mp4', filename: `${get_info.title}.mp4`, quoted: mek })
                    break
                    case 'joox':
                    query = args.join(" ")
		if (!isRegistered) return reply(ind.noregis())
			if (args.length < 1) return reply(`Contoh: ${prefix}joox Kokoronashi`)
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/jooxplay?apikey=RiuApikey&query=${query}`)
                    get_result = get_result.result
		    get_info = get_result.info
		    txt = '*_[ FITUR UP BY MrG{108P} ]_*\n\n'
                    ...
                    case 'fbdl':
                    ini_url = args[0]
                    ini_url = await fetchJson(`http://api.lolhuman.xyz/api/facebook?apikey=RiuApikey&url=${ini_url}`)
